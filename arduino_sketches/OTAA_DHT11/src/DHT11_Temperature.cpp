...
DHT11_Temperature::DHT11_Temperature(char* nomenclature, bool is_analog, bool is_connected, bool is_low_power, uint8_t pin_read, uint8_t pin_power):Sensor(nomenclature,is_analog, is_connected, is_low_power, pin_read, pin_power){
  if (get_is_connected()){
    // start library DHT
    //dht = new DHT11(get_pin_read());
...
void DHT11_Temperature::update_data()
{
  if (get_is_connected()) {
...
double DHT11_Temperature::get_value()
{
  uint8_t retry=4;
  Serial.println("temp");
